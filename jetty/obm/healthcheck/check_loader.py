...
class CheckLoader(object):
    ...
    def __init__(self, checks_path, log=None):
        self._checks = self._check_modules_generator(checks_path)

    def _check_modules_generator(self, checks_path):
        for file in os.listdir(checks_path):
            if os.path.splitext(file)[1].lower() == '.py':
                module = imp.load_source(os.path.splitext(file)[0], os.path.join(checks_path, file))
