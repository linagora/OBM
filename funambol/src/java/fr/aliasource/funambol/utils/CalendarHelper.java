...
// ------------ Public Methods -----------
	 ...
	 * @return
	 */
	public static RecurrencePattern getRecurrence(Date dstart, Date dend, EventRecurrence obmrec) {
		...
		try {
			if (obmrec.getKind().equals("daily")) {
				...
				result = RecurrencePattern.getDailyRecurrencePattern(
						interval, sPatternStart, sPatternEnd, noEndDate);
			
			} else if (obmrec.getKind().equals("weekly")) {
				
				result = RecurrencePattern.getWeeklyRecurrencePattern(
						interval, dayOfWeekMask, sPatternStart, sPatternEnd, noEndDate);
			
			} else if (obmrec.getKind().equals("monthlybydate")) {
			
				result = RecurrencePattern.getMonthlyRecurrencePattern(
						interval, getDayOfMonth(dstart), sPatternStart, sPatternEnd, noEndDate);
				
			} else if (obmrec.getKind().equals("monthlybyday")) {
				
				result = RecurrencePattern.getMonthNthRecurrencePattern(
						interval, getDayOfWeek(dstart), getNthDay(dstart), sPatternStart, sPatternEnd);
				
			} else if (obmrec.getKind().equals("yearly")) {
				
				result = RecurrencePattern.getYearlyRecurrencePattern(
						interval, getDayOfMonth(dstart), getMonthOfYear(dstart), sPatternStart, sPatternEnd, noEndDate);
	 ...
	 * @return
	 */
	public static EventRecurrence getRecurrenceFromFoundation(RecurrencePattern rec, Date dend) {
