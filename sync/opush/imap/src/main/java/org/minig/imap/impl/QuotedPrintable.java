...
/**
 * Implementation of QuotedPrintable en- and decoding methods. QuotedPrintable
 * encoding is used to transform a text that contains non-US-ASCII characters
 * into a US-ASCII compatible text while maintaining human readability as far as
 * possible. <br>
 * <b>RFC(s):</b> 2045
 */
public class QuotedPrintable {
	...
	private static final Pattern packPattern = Pattern
			.compile("=([0-9a-fA-F\r][0-9a-fA-F\n])");
	 ...
	 *            {@link CharSequence}
	 * @param charset
	 *            The {@link Charset} of the decoded text
	...
	public static CharSequence decode(CharSequence input, Charset charset) {
		...
		while (matcher.find()) {
			try {
				...
				buffer.put(input.subSequence(lastMatchEnd, matcher.start())
		...
		try {
			// Add the bytes from the part that isn't encoded to the buffer
			buffer.put(input.subSequence(lastMatchEnd, input.length())
					.toString().getBytes("US-ASCII"));
		} catch (UnsupportedEncodingException e) {
			// Never happens, because US-ASCII is always shipped
		}
	...
	/**
	 * Encodes a CharSequence with QuotedPrintable Coding. <br>
	 * <b>Note:</b> The input is transformed to canonical form before encoding
	 * 
	 * @param input
	 *            The input text in form of a {@link CharSequence}
	 * @param charset
	 *            The charactercoding that the input uses (e.g. ISO-8859-1)
	 * @return a US-ASCII compatible {@link StringBuilder}
	 */
	public static StringBuilder encode(CharSequence input, Charset charset) {
		...
		for (int i = 0; i < input.length(); i++) {
			...
			// c = {9,32} but not at the end of a line or if they are char at
			// linePos 74
