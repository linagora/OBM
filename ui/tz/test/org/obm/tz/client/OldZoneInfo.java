 ...
 * <code>java.util.TimeZone</code> interface and also provides work alikes for
 * unix time conversion functions. Unlike <code>java.util.SimpleTimeZone</code>,
 * this implementation supports historical daylight savings time changes and
 * leap seconds.
 * <p>
 * Unfortunately, the TimeZone API does not support giving a unique name to
 * positive leap seconds (for example, 1998 Dec 31 23:59:60 UTC). As a result, a
 * positive leap second has the same HMS representation as the previous second
 * when using <code>java.util.GregorianCalendar</code>.
 * <p>
 * Even more unfortunately, <code>java.text.SimpleDateFormat</code> decides
 * which TimeZone name to use by comparing the <code>DST_OFFSET</code> calendar
 * field to zero. <code>GregorianCalendar</code> computes this by subtracting
 * <code>getRawOffset()</code> from <code>getOffset()</code>. Of course, these
 * are never equal once leapseconds kick in, so beginning in 1972,
 * SimpleDateFormat always (incorrectly) uses the daylight time abbreviation
 * with this TimeZone implementation.
 * <p>
 * This code is based loosely on the unix localtime package version 4.1. Rules
 * for each timezone are stored in binary form in the <code>/etc/zoneinfo</code>
 * directory. These binary files are produced by the zoneinfo compiler,
 * <code>zic</code>, included with the localtime package as C source. I have not
 * yet ported the compiler to Java. The format accomodates historical timezone
 * changes (e.g. war time and the 1987 change in the US), and leapseconds.
 * <p>
 * Localtime format uses signed 32-bit values, so it peters out in 2038. This
 * can be extended by noting that each table has monotonically increasing keys -
 * hence the high order bits can be implied. However, timezone changes are
 * listed exhaustively (rules are interpreted by the zoneinfo compiler), so the
 * tables would be quite large if extended to the full range of 64-bit Java time
 * values. I propose that the timezone files can be gradually extended, becoming
 * larger and larger as required, until a better solution is invented. There is
 * no point at which things suddenly break.
 * <p>
 * I have not yet implemented the implicitly extended table, so this version
 * will break with regards to determining daylight savings time and accumulating
 * leapseconds beginning in 2039. The main purpose of this implementation is to
 * point out deficiences in the JDK classes. Another deficiency not mentioned
 * above is that a TimeZone can have more than two abbreviations. For example,
 * Eastern time includes EST,EDT,EWT.
 * <p>
 * The best way to make this actually useful, besides extending the range beyond
 * 2038, is probably to extend, fix, or replace GregorianCalendar (to support
 * minutes with leap seconds and the correct DST_OFFSET). The two abbreviation
 * limit can be fixed by adding a ZONE_INDEX field to
 * <code>java.util.Calendar</code> and using it in <code>SimpleDateFormat</code>.
 * 
 * @author Stuart D. Gathman Copyright (C) 1998 Business Management Systems,
 *         Inc.
 */
public class OldZoneInfo extends TimeZone {
	...
	/**
	 * Timezone type description. E.g. EST or EDT. This should be an inner
	 * class, but 1.1 JDK compiler freaks out when using blank final with inner
	 * classes. It is permissible for a package private class not used from any
	 * other class in the package to be in the same source file. However, some
	 * java IDE tools freak out over this.
	 * 
	 * @author Stuart D. Gathman
	 */
	private class TZType implements Serializable {
		private static final long serialVersionUID = 1L;
	...
	public OldZoneInfo(File f) throws IOException {
		...
		try {
			...
			for (int i = 0; i < typecnt; ++i) {
				...
				dst[i] = ds.readByte();
				idx[i] = ds.readByte();
				System.out.println("offset: " + offset[i] + " dst: " + dst[i]
						+ " idx: " + idx[i]);
			}
			...
			for (int i = 0; i < typecnt; ++i) {
				...
				while (str[end] != 0)
					++end;
				tz[i] = new TZType(new String(str, pos, end - pos), offset[i],
						dst[i] != 0);
			}
		...
		// non-integer time zones were used, e.g. if GMT-2:33 was used until
		// 1918.

		// This loop, based on a suggestion by Ophir Bleibergh, tries to find a
		// non-DST rule close to the current time. This is somewhat of a hack,
		// but
		// much better than the previous behavior in this case.

		// Tricky: we need to get either the next or previous non-dst TZ
		// We shall take the future non-dst value, by trying to add 3 months at
		// a
	...
	/**
	 * Return the ZoneInfo for local time on this machine. For unix, we read
	 * /etc/localtime, which is a link to the proper zoneinfo file.
	 */
	public OldZoneInfo() throws IOException {
	 ...
	 * 
	 * @param tzname
	 *            the name of the timezone to read
	 */
	public OldZoneInfo(String tzname) throws IOException {
	...
	/**
	 * Return the offset from UT for a calendar date and time. The calendar time
	 * we are passed is always computed using getRawOffset().
	 */
	public int getOffset(int era, int year, int month, int day, int dow,
			int millis) {
	...
	/**
	 * Return true if a particular instant is considered part of daylight time
	 * in this timezone.
	 */
	...
	/**
	 * Return true if this timezone has transitions between various offsets from
	 * UT, such as standard time and daylight time.
	 */
	...
	public static class tm {
		...
		public int tm_min;
		/**
		 * Second of minute, 0 - 60. Note that value may be 60 on a leap second.
		 */
		public int tm_sec;
		...
		public String toString() {
			return f.format(tm_mon + 1) + '/' + f0.format(tm_mday) + '/'
					+ f0.format(tm_year + 1900) + ' ' + f.format(tm_hour) + ':'
					+ f0.format(tm_min) + ':' + f0.format(tm_sec) + ' '
					+ (tm_zone == null ? "NUL" : tm_zone);
		}
	...
	/**
	 * Compute UTC from clock. This includes leap second corrections if compiled
	 * into the current timezone file.
	 * 
	 * @param clock
	 *            seconds since 1970
	 * @return a new tm object with all time fields computed.
	...
	/**
	 * Compute local time from seconds since the epoch, storing into an existing
	 * tm object.
	 * 
	 * @param clock
	 *            seconds since 1970
	 * @param t
	 *            a tm object to store the computed time fields
	...
	/**
	 * Calculate seconds since the epoch, the reverse of localtime(). Unused
	 * fields are computed and stored in <code>yourtm</code>.
	 * 
	 * @param yourtm
	 *            The tm_year,tm_mon,tm_mday,tm_hour,tm_min,tm_sec fields are
	 *            used and validated. Other fields are computed.
	 * @throws IllegalArgumentException
	 *             If used fields are invalid.
	...
	private long mktime(tm yourtm, boolean raw) {
		...
		for (;;) {
			...
			// System.err.println(mytm.toString() + ", " + t + ", " +
			// direction);
			if (bits-- < 0)
	...
	public static void main(String[] argv) throws Exception {
		OldZoneInfo tz = new OldZoneInfo("EST5EDT");
	 ...
	 * GregorianCalendar times before and after each timezone and leapsecond
	 * transition, and at the minimum and maximum unix format times.
	 */
	static class TZDump {
		private String zone;
		private OldZoneInfo tz;
		...
		private void dump(int time) {
			tm t = tz.localtime(time);
			System.out.println(zone + ' ' + tz.gmtime(time) + " = "
					+ dumplcl(time));
		}
		...
		public static void dumpzone(String tzname) throws IOException {
		...
		/**
		 * Dump DST and leapsecond transitions for each timezone on the command
		 * line.
		 */
		public static void main(String[] argv) throws Exception {
			if (argv.length == 0)
