...
/**
 * Timezone type description. E.g. EST or EDT. This should be an inner class,
 * but 1.1 JDK compiler freaks out when using blank final with inner classes. It
 * is permissible for a package private class not used from any other class in
 * the package to be in the same source file. However, some java IDE tools freak
 * out over this.
 * 
...
public class ZoneInfo {
	...
	public ZoneInfo(byte[] s) {
		DataStream ds = new DataStream(s);
		...
		try {
			...
			for (int i = 0; i < typecnt; ++i) {
				...
				byte[] ret = new byte[end - pos];
				System.arraycopy(str, pos, ret, 0, ret.length);
				StringBuilder bu = new StringBuilder();
				for (byte b : ret) {
					bu.append(Byte.toString(b));
				}

				tz[i] = new TZType(bu.toString(), offset[i], dst[i] != 0);
			}
