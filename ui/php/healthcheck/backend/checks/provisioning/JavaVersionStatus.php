...
class JavaVersionStatus extends AbstractMultiDomainStatus {
  ...
  const JAVA_VERSION_URL = "http://%HOST%:8086/healthcheck/java/version";
  const JAVA_VMNAME_URL = "http://%HOST%:8086/healthcheck/java/vmname";
  
  public function executeForDomain($domain) {
    $servers = of_domain_get_domain_provisioningserver($domain["id"]);
    ...
    foreach ($servers as $server) {
      ...
      return new CheckResult(CheckStatus::ERROR, array("Provisioning server at '" . $host["ip"] . "' for domain '" . $domain["name"] . "' isn't reachable"));
    }
  }
  ...
  private static function checkResult($versionOk, $version, $expectedVersion) {
      return $versionOk ? new CheckResult(CheckStatus::OK) : new CheckResult(CheckStatus::ERROR, array("Provisioning server at '" . $host["ip"] . "' for domain '" . $domain["name"] . "' runs Java version '" . $version . "', expecting '" . $expectedVersion . "'."));
  }
}
