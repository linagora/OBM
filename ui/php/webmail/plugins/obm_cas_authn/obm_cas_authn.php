...
/**
 * OBM-ified CAS Authentication
 * This plugin is a derivative of David Warden's cas_authn. The only difference
 * is that it makes it possible to retrieve the id of the user from
 * 'cas_server_login_attribute' in the configuration.
 */

class obm_cas_authn extends rcube_plugin {
...
* @return array modified arguments
    ...
    */
    ...
    function getUser() {
        $cfg = rcmail::get_instance()->config->all();
        $cas_server_login_attribute = $cfg['cas_server_login_attribute'];
        $login = null;
        if(empty($cas_server_login_attribute)) {
          $login = phpCAS::getUser();
        }
        else {
          $login = phpCAS::getAttribute($cas_server_login_attribute);
          if(empty($login)) {
            $login = phpCAS::getUser();
          }
        }
