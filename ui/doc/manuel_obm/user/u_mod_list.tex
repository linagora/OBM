...
Le mode normal propose de siasir graphiquement les critères de ciblage.
...
Les critères de la même fenêtre sont cumulés et liés par des ET logiques.\\

Entre 2 lignes (ou fenêtre de critères) il est possible de choisir le mode de combinaison logique (OU ou ET) et d'inverser le critère (opérateur NOT).
...
\paragraph{Avertissement} Ce mode est réservé aux utilisateurs avertis car le modèle de la base de données OBM étant susceptible d'évoluer, certaines requêtes peuvent ne plus être fonctionnelles après une mise à jour d'OBM.
...
\hline
Normal -> Expert & Les critères saisis sont conservés et transformés en requête SQL qui est reprise.\\
\hline
Expert -> Normal & La requête saisie est perdue, la détermination des critères est à effectuer graphiquement.\\
\hline
\end{tabular}
\vspace{0.3cm}
...
Le module \List comporte 7 sous-menus :
...
\hline
Consulter & Consulter les informations de la liste en cours et du résultat de ciblage \\
\hline
Modifier & Modifier les informations de la liste en cours liste\\
\hline
