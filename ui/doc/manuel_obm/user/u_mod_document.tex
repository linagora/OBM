...
Plusieurs format de documents sont gérés (pdf,jpg,word...). L'utilisateur peut déterminer le format du document de façon automatique ou manuelle en sélectionnant le type \textbf{mime} dans la liste déroulante.\\
...
\hline
Fichier & Fichier à télécharger pour créer le document.\\
\hline
\end{tabular}
\vspace{0.3cm}
...
Par exemple :\\ \textbf{Catégorie 1} Compte rendu de réunion, proposition commerciale, contrat ....\\
...
OBM associe à chaque type MIME une application utilisée pour ouvrir le fichier.\\
...
- via le bouton ''Parcourir'' afin de chercher où se trouve le fichier recherché à partir de la racine selectionnée.\\

\subsection{Les sous-menus du module \doc}

Le module \doc comporte 7 sous-menus (accessibles selon les droits d'accès et le contexte) :\\
...
\hline
Affichage & Personnalisation de l'affichage de la liste des documents (possibilité de masquer ou pas certains champs).\\
\hline
\end{tabular}
...
Ce menu permet de créer un document dans la base documentaire.
...
Si le document peut être supprimé, un bouton ''Supprimer le document'' est alors affiché sinon un message d'avertissement montrant les liens existant avec ce document signale que la suppression est impossible.
...
\paragraph{Gestion des types MIME : } Le type MIME d'un document est défini par 3 informations liées :\\

\begin{itemize}
\item Le label du type MIME, qui une fois choisie donne accès au type MIME du document
\item L'extension, qui une fois choisi donne accès aux extensions de fichier (exemple : .doc,.odt,.gif...) 
\item Le type MIME, qui une fois choisi donne accès à l'application utilisée pour ouvrir un document ayant le type d'extension donné.
\end{itemize}
