...
\hline
\multicolumn{3}{c}{Paramètres globaux}\\
\hline
...
\hline
\$popup\_width & 620 & Largeur en pixels des popus \\ 
\hline
\$popup\_height & 480 & Hauteur en pixels des popus \\ 
\hline
...
\hline
\multicolumn{3}{c}{Paramètres spéciques aux modules}\\
\hline
...
\hline
\$ccontact\_private\_default & false & Indicateur contact sont privés par défaut \\ 
\hline
\$document\_path & /var/www/docs & Chemin du dépôt de document sur disque \\ 
\hline
\$default\_path & /Default & Répertoire par défaut dans le dépot de documents \\ 
\hline
...
\hline
\$tmp\_path & /tmp & Chemin pour stocake temporaire (module import) \\ 
\hline
...
Le tableau \$cgp\_show[``section''] définit les sections utilisées par OBM et pour chacune d'elle son lien (url) par défaut.
...
A partir d'OBM 0.9.0, il suffit de ne pas définir la section.\\
...
Liste des sections définies par défaut sont :\\
\begin{itemize}
\item com
\item prod
\item compta
...
A partir d'OBM 0.9.0, aucune section n'est définie en dur dans le code et l'administrateur définit les sections et leurs modules uniquement via le fichier de configuration.


\subsection{Le paramétrage des modules visibles}
\label{cgp_show_2}

Le tableau \$cgp\_show[``module''] définit pour chaque module si celui-ci doit être affiché et dans quelle section.
