\section{Installation pas à pas d'OBM 0.9.x}

Cette documentation est valide pour la version 0.9.0. Seules les
manipulations spécifiques à OBM sont décrites. Pour l'installation de
PHP, MySQL, PostgreSQL, Apache ou autres composants libres, merci de vous référer
...
\shadowbox{
  \begin{minipage}{13cm}
\begin{verbatim}
wget http://obm.aliacom.fr/obm-archives/obm-0.9.0.tar.gz
tar -C /var/www -xzvf obm-0.9.0.tar.gz
cd /var/www
ln -s obm-0.9.0 obm
\end{verbatim}
  \end{minipage}
...
\begin{description}
\item[sur debian] \fichier{/etc/php/apache/php.ini}
...
La directive include\_path doit être renseignée ainsi : 
...
Éditer le fichier \fichier{httpd.conf} (ou \fichier{srm.conf}) et
ajouter :
...
\subsection{Création de la base de données OBM}
...
répertoire \fichier{scripts/0.9}.
...
\fichier{scripts/0.9/install\_obmdb\_0.9.mysql.sh} (ou
\fichier{scripts/0.9/install\_obmdb\_0.9.pgsql.sh}) pour changer la
...
Le script \fichier{install\_obmdb\_0.9.mysql.sh} gère les différentes
...
\begin{verbatim}
cd obm/scripts/0.9
emacs install_obmdb_0.9.mysql.sh
./install_obmdb_0.9.mysql.sh
\end{verbatim}
  \end{minipage}
}
...
\subsubsection{Pour OBM depuis la version 0.8.3}

Si votre server PostgreSQL n'est pas configuré pour le jeu de
caractères latin1, éditez le fichier \fichier{scripts/0.9/postgres-pre.sql}
...
\subsubsection{Pour toutes les versions d'OBM}

Le script \fichier{install\_obmdb\_0.9.pgsql.sh} gère toutes les
...
\begin{verbatim}
cd obm/scripts/0.9
emacs install_obmdb_0.9_pgsql.sh
./install_obmdb_0.9_pgsql.sh
\end{verbatim}
  \end{minipage}
}
