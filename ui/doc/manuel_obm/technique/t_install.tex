\section{Installation pas à pas d'OBM 1.0.x}

Cette documentation est valide pour la version 1.0.0. Seules les
manipulations spécifiques à OBM sont décrites. Pour l'installation de
PHP, MySQL, PostgreSQL, Apache ou autres composants libres, merci de vous référer à la documentation de votre distribution.
...
\shadowbox{
  \begin{minipage}{13cm}
\begin{verbatim}
wget http://obm.aliacom.fr/obm-archives/obm-1.0.0.tar.gz
tar -C /var/www -xzvf obm-1.0.0.tar.gz
cd /var/www
ln -s obm-1.0.0 obm
\end{verbatim}
  \end{minipage}
...
répertoire \fichier{scripts/1.0}.
...
\fichier{scripts/1.0/install\_obmdb\_1.0.mysql.sh} (ou
\fichier{scripts/1.0/install\_obmdb\_1.0.pgsql.sh}) pour changer la
...
Le script \fichier{install\_obmdb\_1.0.mysql.sh} gère les différentes
...
\begin{verbatim}
cd obm/scripts/1.0
emacs install_obmdb_1.0.mysql.sh
./install_obmdb_1.0.mysql.sh
\end{verbatim}
  \end{minipage}
}
...
caractères latin1, éditez le fichier \fichier{scripts/1.0/postgres-pre.sql}
...
Le script \fichier{install\_obmdb\_1.0.pgsql.sh} gère toutes les
...
\begin{verbatim}
cd obm/scripts/1.0
emacs install_obmdb_1.0_pgsql.sh
./install_obmdb_1.0_pgsql.sh
\end{verbatim}
  \end{minipage}
}
