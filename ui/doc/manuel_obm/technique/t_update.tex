...
\paragraph{Règles :} l'IHM n'écrit JAMAIS dans les tables de production, celles-ci sont gérées par l'automate. L'automate n'accède jamais aux tables standards.
...
\begin{tabular}{|p{1,5cm}|p{2,6cm}|p{3,5cm}|p{6,5cm}|}
\hline
\textbf{Domaine} & \textbf{Interface : Màj} & \textbf{actions} & \textbf{paramètres automate} \\
\hline
\multirow{2}{2cm}{0} & Globale & Globale à l'application &
--all
\\
\cline{2-4}
...
\subsubsection{Mise à jour selon portée}
...
\begin{itemize}
\item verrou IHM est OK (\db{ObmInfo.update\_state} = 0)
...
Automate lancé &
Donnée en base de donnée : \db{ObmInfo.update\_state} (1=en cours, 0 sinon)
