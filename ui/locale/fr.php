<?php
/*
 +-------------------------------------------------------------------------+
 |  Copyright (c) 1997-2009 OBM.org project members team                   |
 |                                                                         |
 | This program is free software; you can redistribute it and/or           |
 | modify it under the terms of the GNU General Public License             |
 | as published by the Free Software Foundation; version 2                 |
 | of the License.                                                         |
 |                                                                         |
 | This program is distributed in the hope that it will be useful,         |
 | but WITHOUT ANY WARRANTY; without even the implied warranty of          |
 | MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the           |
 | GNU General Public License for more details.                            |
 +-------------------------------------------------------------------------+
 | http://www.obm.org                                                      |
 +-------------------------------------------------------------------------+
*/
?>
<?php

return array(
  //FIXME : Move this into a specific module file
  'NEEDS-ACTION' => 'mis en attente',
  'ACCEPTED' => 'accepté',
  'DECLINED' => 'refusé',
  '%sender%, New event on OBM: %title%' => '%sender%, Nouvel événement sur OBM : %title%',
  '%sender%, Event cancelled on OBM: %title%' => '%sender%, Evénement annulé sur OBM : %title%',
  '%sender%, Event updated on OBM: %title%' => '%sender%, Mise à jour d\'un événement sur OBM : %title%',
  'New resource reservation on OBM: %title%' => 'Nouvelle réservation de ressource sur OBM : %title%',
  'Resource reservation updated on OBM: %title%' => 'Mise à jour d\'une réservation de ressource sur OBM : %title%',
  'New resource reservation on OBM: %title%' => 'Nouvelle réservation de ressource sur OBM : %title%',
  'Exploitation report : %name%' => 'Rapport d\'exploitation : %name%',
  '%displayname% validation confirmation' => 'Confirmation de la validation de l\'utilisateur %displayname%',
  'Resource participation updated on OBM: %title%' => 'La réservation d\'une resource à été statuée dans OBM : %title%',
  'Participation updated on OBM: %title%' => 'Mise à jour de participation dans OBM : %title%',
  'User %displayname% is to be validated' => 'L\'utilisateur %displayname doit être validé',
  '%lastname% %firstname%' => '%firstname% %lastname%',
  'Address' => 'Adresse',
  'Addresses' => 'Adresses',
  'Advanced search' => 'Recherche avancée',
  'Archive' => 'Archive',
  'Archived' => 'Archivé',
  'Cancel' => 'Annuler',
  'Close' => 'Fermer',
  'Comment' => 'Commentaires', 
  'Company' =>  'Société',
  'Consult' => 'Consulter',
  'Country' => 'Pays',
  'Delete' => 'Supprimer',
  'Displayname' => 'Nom complet',
  'Email' => 'Courriel', 
  'Emails' => 'Courriels', 
  'Export' => 'Exporter',
  'Firstname' => 'Prénom',
  'Hide advanced search' => 'Cacher la recherche avancée',
  'Import' => 'Importer',
  'Instant messaging' => 'Messagerie instantanée',
  'Instant messagings' => 'Messageries instantanées',
  'Lastname' => 'Nom',
  'Mailing activated' => 'Accepte les courriels',
  'No' => 'Non',
  'Permission denied' => 'Permissions insuffisantes',
  'Phone' => 'Téléphone',
  'Phones' => 'Téléphones',
  'Right management'  => 'Gestion des droits',
  'Save' => 'Sauvegarder',
  'Search results' => 'Résultats de recherche',
  'Search' => 'Rechercher',
  'Subscribed for newsletter' => "Inscrit à l'infolettre",
  'Title' => 'Titre',
  'Town' => 'Ville',
  'Update' => 'Mettre à jour',
  'Validate' => 'Valider',
  'Website' => 'Site web',
  'Websites' => 'Sites web',
  'Yes' => 'Oui',
  'Zip code' => 'Code postal',
  'Street' => 'Rue',
  'Express postal' => 'Cedex',
  'Function' => 'Fonction'
);
