...
Les types OBM déclarés sont :

=over 4

=item $ROOT =E<gt> C<organization>, C<dcObject>

B<structurel> - définit le type OBM ROOT, représentant la racine de l'arbre LDAP ;

=item $NODE =E<gt> C<organizationalUnit>

B<structurel> - définit le type OBM NODE, représentant un noeud de l'arbre LDAP ;

=item $DOMAINROOT =E<gt> C<organizational>, C<dcObject>

B<structurel> - définit le type OBM DOMAINROOT, représentant un noeud de
...
B<structurel> - définit le type OBM POSIXUSERS, représentant un utilisateur POSIX ;

=item $SAMBAUSERS =E<gt> C<sambaSamAccount>

B<non structurel> - définit le type OBM SAMBAUSERS, représentant un utilisateur SAMBA ;
...
(Samba) ou par OBM pour se connecter à certains services (LDAP, MySql) ;

=item $POSIXGROUPS =E<gt> C<posixGroup>

B<structurel> - définit le type OBM POSIXGROUPS, représentant un groupe POSIX ;

=item $SAMBAGROUPS =E<gt> C<sambaGroupMapping>

B<non structurel> - définit le type OBM SAMBAGROUPS, représentant un groups SAMBA ;

=item $SAMBAHOSTS =E<gt> C<inetOrgPerson>, C<posixAccount>, C<shadowAccount>, C<sambaSamAccount>

B<structurel> - définit le type OBM SAMBAHOSTS, représentant un compte machine dans un
domaine Samba ;

=item $ADDRESSBOOK =E<gt> C<groupOfNames>, C<aliaminGroups>

B<non structurel> - définit le type OBM ADDRESSBOOK permettant de stocker des contacts
dans l'annuaire LDAP - expérimental 20040930 ;

=item $SAMBADOMAIN =E<gt> C<sambaDomain>

B<structurel> - définit le type OBM SAMBADOMAIN, représentant un des nombreux objets LDAP
...
B<structurel> - définit le type OBM SAMBAFREEUNIXID, représentant un des nombreux objets LDAP
nécessaires au fonctionnement de Samba en PDC. Il 'sert' à stocker les prochains UID et GID
libres - à approfondir 20040930 -- non implémenté 20070414 ;

=item $MAILSERVER =E<gt> C<aliaminPostfix>

B<non structurel> - définit le type OBM MAILSERVER, représentant la
configuration du service de messagerie dans l'annuaire LDAP ;

=item $MAILSHARE =E<gt> C<mailShare>

B<structurel> - définit le type OBM MAILSHARE représentant les
répertoires partagés de messagerie ;

=item $GROUPOFNAME =E<gt> C<groupOfNames>

B<structurel> - définit le type OBM GROUPOFNAME représentant les
...
objet de ce type - facultative ;

=item B<get_db_value> : référence à une fonction permettant de charger les
valeurs depuis la base de données OBM - nécessaire ;

=item B<create_ldap> : référence à une fonction permettant de créer l'objet LDAP
correpondant aux valeurs de la base OBM - nécessaire ;

=item B<update_ldap> : référence à une fonction permettant de mettre à jour
l'entité LDAP - nécessaire ;

=item B<update_passwd> : référence à une fonction permettant de mettre à jour le
mot de passe de l'entité LDAP - facultative.
...
=item B<domain_id> : indice du tableau C<domainList> indiquant à quel domaine
OBM se rapporte cette entité ;

=item B<ldap_server> : description du serveur LDAP qui héberge le noeud et son
sous arbre. Cette clé ne peut-être utilisé qu'au niveau d'une entité de type
C<branche>. Le serveur est décrit par :

=over 4

=item B<server> : nom ou adresse IP du serveur LDAP a contacter ;

=item B<login> : nom de l'utilisateur système administrateur de cet annuaire ;

=item B<passwd> : mot de passe de l'administrateur ;

=item B<conn> : objet de type C<Net::LDAP> qui est la connexion établie avec le
serveur.
