...
psql -U ${user} -h ${host} ${db} -f \
  "updates/update-2.5.7~2.pgsql.sql" >> /tmp/data_insert.log 2>&1

psql -U ${user} -h ${host} ${db} -f \
  "updates/update-2.5.8~1.pgsql.sql" >> /tmp/data_insert.log 2>&1

psql -U ${user} -h ${host} ${db} -f \
  "updates/update-2.5.8~2.pgsql.sql" >> /tmp/data_insert.log 2>&1

psql -U ${user} -h ${host} ${db} -f \
  "updates/update-2.5.8~3.pgsql.sql" >> /tmp/data_insert.log 2>&1

psql -U ${user} -h ${host} ${db} -f \
  "updates/update-2.5.9~1.pgsql.sql" >> /tmp/data_insert.log 2>&1
