...
mysql -u $user -p$pw $db < create_obmdb_3.0.mysql.sql
test $? -eq 0 || {
...
mysql --default-character-set='UTF8' -u $user -p$pw $db < obmdb_default_values_3.0.sql
test $? -eq 0 || {
    echo "error running mysql script"
    exit 1
}
...
mysql --default-character-set='UTF8' -u $user -p$pw $db < data-$obm_lang/obmdb_ref_3.0.sql
test $? -eq 0 || {
    echo "error running mysql script"
    exit 1
}
...
mysql --default-character-set='UTF8' -u $user -p$pw $db < data-$obm_lang/obmdb_nafcode_3.0.sql
test $? -eq 0 || {
    echo "error running mysql script"
    exit 1
}
...
mysql --default-character-set='UTF8' -u $user -p$pw $db < obmdb_prefs_values_3.0.sql
test $? -eq 0 || {
    echo "error running mysql script"
    exit 1
}
...
mysql --default-character-set='UTF8' -u ${user} -p$pw ${db} \
  < "updates/update-3.0.0~1.mysql.sql" >> /tmp/data_insert.log 2>&1
