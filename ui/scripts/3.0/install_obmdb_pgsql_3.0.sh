...
create_obmdb_3.0.pgsql.sql > /tmp/data_insert.log 2>&1
...
psql -U ${user} -h ${host} ${db} -f \
obmdb_default_values_3.0.sql >> /tmp/data_insert.log 2>&1
grep -i error /tmp/data_insert.log && {
    echo "error in pg script"
    exit 1
}
...
psql -U ${user} -h ${host} ${db} -f \
data-${obm_lang}/obmdb_ref_3.0.sql >> /tmp/data_insert.log 2>&1
grep -i error /tmp/data_insert.log && {
    echo "error in pg script"
    exit 1
}
...
psql -U ${user} -h ${host} ${db} -f \
data-${obm_lang}/obmdb_nafcode_3.0.sql >> /tmp/data_insert.log 2>&1
grep -i error /tmp/data_insert.log && {
    echo "error in pg script"
    exit 1
}
...
psql -U ${user} -h ${host} ${db} -f \
obmdb_prefs_values_3.0.sql >> /tmp/data_insert.log 2>&1
grep -i error /tmp/data_insert.log && {
    echo "error in pg script"
    exit 1
}
...
psql -U ${user} -h ${host} ${db} -f \
  "updates/update-3.0.0~1.pgsql.sql" >> /tmp/data_insert.log 2>&1
