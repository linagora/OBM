#!/bin/bash
###############################################################################
# OBM - File : install_obmdb_3.0.sh                                           #
#     - Desc : OBM Database 3.0 installation script                           #
###############################################################################
...
./install_document_3.0.sh

if [ -z $installation_type ] || [ $installation_type != 'filldata' ]; then
	./install_obmdb_${DBTYPE}_3.0.sh ${DB} ${U} ${P} ${OBM_LANG} "full"
else
	./install_obmdb_${DBTYPE}_3.0.sh ${DB} ${U} ${P} ${OBM_LANG} "filldata"
fi
