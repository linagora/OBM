...
psql -U $U $DB < create_obmdb_1.2.pgsql.sql
...
cat postgres-pre.sql data-$DATA_LANG/obmdb_ref_1.2.sql | psql -U $U $DB

# Company Naf Code data insertion
echo "  Company Naf Code data insertion"
cat postgres-pre.sql data-$DATA_LANG/obmdb_nafcode_1.2.sql | psql -U $U $DB

# Test data insertion
echo "  Test data insertion"
cat postgres-pre.sql obmdb_test_values_1.2.sql | psql -U $U $DB

# Default preferences data insertion
echo "Default preferences data insertion"
cat postgres-pre.sql obmdb_default_values_1.2.sql | psql -U $U $DB 
