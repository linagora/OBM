#!/bin/sh
###############################################################################
# OBM - File : install_obmdb_0.7.pgsql.sh                                     #
#     - Desc : PostgreSQL Database 0.7 installation script                    #
# 2003-12-13 ALIACOM                                                          #
###############################################################################
# $Id$
###############################################################################
...
psql -U $U $DB < create_obmdb_0.7.pgsql.sql

# Dictionnary data insertion
echo "Dictionnary data insertion"
psql -U $U $DB < create_obmdb_0.7_fr.mysql.sql

# Test data insertion
echo "Test data insertion"
psql -U $U $DB < obmdb_test_values_0.7.sql

# Default preferences data insertion
echo "Default preferences data insertion"
psql -U $U $DB < obmdb_default_values_0.7.sql
