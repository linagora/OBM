...
psql -U ${U} -h localhost ${DB} -f ./update-2.3-2.4.pgsql.sql >>/tmp/update_obm.log 2>&1
success=$?
test ${success} -eq 0 || {
    echo "Error running 2.3 to 2.4 upgrade script."
    exit 1
}
echo "[DONE]"

echo "Reloading default 2.4 preferences..."
psql -U ${U} -h localhost ${DB} -f ./obmdb_prefs_values_2.4.sql >>/tmp/update_obm.log 2>&1
success=$?
test ${success} -eq 0 || {
