...
public class EncodedWord {
	...
	private static final Pattern encodedWordPattern = Pattern
			.compile("=\\?([^?]+)\\?([bBqQ])\\?([^?]+)\\?=");
	...
	private static final Pattern wordTokenizerPattern = Pattern
			.compile("\\b([^\\s]+[\\s]*)");
	...
	public static StringBuilder decode(CharSequence input) {
		...
		while (matcher.find()) {
			CharSequence inbetween = input.subSequence(lastMatchEnd, matcher
					.start());
	...
	public static StringBuilder encode(CharSequence input, Charset charset,
		...
		for (int i = 0; i < words.size(); i++) {
			int[] act = words.get(i);
			if (last != null
					&& (last[2] + act[2] + (act[0] - last[1]) * normalChar < maxLength)
					&& (act[0] - last[1]) < 10) {
				words.remove(i--);
				last[1] = act[1];
		...
		while (it.hasNext()) {
			...
			if (type == QUOTED_PRINTABLE) {
				...
				encodedPart = Base64.encodeBase64String(
						charset.encode(CharBuffer.wrap(rawWord)).array());
			}
