...
public class CalendarBindingImpl implements ICalendar {
	...
	public Event modifyEvent(AccessToken token, String calendar, Event event, boolean updateAttendees, boolean notification) 
		...
		try {
			...
			Event forceEndRepeatEvent = forceEndRepeatToLastOccurrence(event);
			
			Event toReturn = modifyEvent(token, calendar, forceEndRepeatEvent, updateAttendees, notification, before);
	...
	private String getRecurrenceIdToIcalFormat(Date recurrenceId) {
		net.fortuna.ical4j.model.DateTime recurrenceIdToIcalFormat = new net.fortuna.ical4j.model.DateTime(recurrenceId);
