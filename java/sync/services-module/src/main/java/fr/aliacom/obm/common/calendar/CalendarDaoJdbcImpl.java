...
public class CalendarDaoJdbcImpl implements CalendarDao {
	...
	@Override
	public Collection<Event> getResourceEvents(ResourceInfo resourceInfo, SyncRange syncRange)
		...
		try {
			...
			EventLoader loader = EventLoader.builder().connection(conn).domainName(domainName).calendar(cal).
				...
				withoutDeclinedAttendee(resourceAtt).withoutMovedExceptions(true).build();
	...
	private void replaceDeclinedEventExceptionByException(ObmUser calendarUser,
		...
		for(Event event: changedEvent) {
			if(event.isRecurrent()) {
				event.getRecurrence().replaceUnattendedEventExceptionByException(calendarUser.getEmailAtDomain());
			}
		}
	}
