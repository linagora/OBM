...
public class DatabaseConnectionProviderImplTest {
	...
	public void setUp() {
		...
		databaseDriverConfigurationFactory = control.createMock(DatabaseDriverConfigurationFactory.class);
		
		expect(databaseDriverConfigurationFactory.create()).andReturn(new PostgresDriverConfiguration());
	}

	private DatabaseConnectionProviderImpl databaseConnectionProvider() {
		dbConnProvider = new DatabaseConnectionProviderImpl(
				transactionAttributeBinder,
				new PoolingDataSourceDecorator(
						databaseDriverConfigurationFactory,
