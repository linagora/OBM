...
public class RootHandler implements HealthCheckHandler {
	...
	public Set<EndpointDescription> root() {
		...
		for (Class<?> handlerClass : classes) {
			...
			Resource resource = Resource.from(handlerClass);
			String resourcePath = resource.getPath();
			for (Resource childResource: resource.getChildResources()) {
				for (ResourceMethod endpoint: childResource.getAllMethods()) {
					endpoints.add(new EndpointDescription(endpoint.getHttpMethod(), resourcePath + '/' + childResource.getPath()));
				}
			}
		}
