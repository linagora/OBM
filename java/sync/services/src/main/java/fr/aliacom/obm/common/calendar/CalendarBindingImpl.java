...
public class CalendarBindingImpl implements ICalendar {
	...
	private void cancelInternalEvent(AccessToken token,
		...
		logger.info("Calendar : event[" + uid + "] removed");
	...
	private Event cancelInternalEventByExtId(AccessToken token, ObmUser obmUser, Event event, boolean notification) throws SQLException {
		...
		logger.info("Calendar : event[" + extId + "] removed");
	...
	private boolean changeParticipationForRecursiveEvent(AccessToken token,
		...
		if (currentEvent != null) {
			if (recurrenceId != null) {
				changed = participationService.changeOnOccurrence(token, extId, recurrenceId, participation, sequence, calendarOwner, currentEvent);
			} else {
				changed = participationService.changeOnEvent(token, extId, participation, sequence, calendarOwner, currentEvent);
			}
		}
	...
	private Attendee findTypedAttendee(Attendee attendee, ObmUser owner) {
		return attendeeService.findAttendee(
				attendee.getDisplayName(), attendee.getEmail(), true, owner.getDomain(), owner.getUid());
	}
