...
CREATE TABLE opush_sync_perms (
    ...
    CONSTRAINT opush_sync_perms_device_id_fkey FOREIGN KEY (device_id) REFERENCES opush_device(id) ON DELETE CASCADE,
    CONSTRAINT opush_sync_perms_owner_fkey FOREIGN KEY (owner) REFERENCES userobm(userobm_id) ON DELETE CASCADE,
    CONSTRAINT opush_sync_perms_policy_fkey FOREIGN KEY (policy) REFERENCES opush_sec_policy(id) ON DELETE CASCADE
);
