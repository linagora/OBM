...
public class UIDFetchHeadersCommandTest {
	
	@Test
	public void testHandleMultipleResponsesWithOnlyOneCorresponding() {
		IMAPResponse response = new IMAPResponse("OK", "* OK [COPY 23 1 2]");
		IMAPResponse response2 = new IMAPResponse("OK", "* 1 FETCH (UID 12 BODY[HEADER.FIELDS (DATE FROM)] {66}"+
			"From: userb@antoine.org"+
			"Date: Fri, 14 Dec 2012 10:46:28 +0100"+
			")");
		IMAPResponse response3 = new IMAPResponse("OK", "");
		
		UIDFetchHeadersCommand command = new UIDFetchHeadersCommand(ImmutableList.of(12l), new String[] { "DATE", "FROM" });
