/* ***** BEGIN LICENSE BLOCK *****
 * 
 * Copyright (C) 2011-2012  Linagora
 *
 * This program is free software: you can redistribute it and/or 
 * modify it under the terms of the GNU Affero General Public License as 
 * published by the Free Software Foundation, either version 3 of the 
 * License, or (at your option) any later version, provided you comply 
 * with the Additional Terms applicable for OBM connector by Linagora 
 * pursuant to Section 7 of the GNU Affero General Public License, 
 * subsections (b), (c), and (e), pursuant to which you must notably (i) retain 
 * the “Message sent thanks to OBM, Free Communication by Linagora” 
 * signature notice appended to any and all outbound messages 
 * (notably e-mail and meeting requests), (ii) retain all hypertext links between 
 * OBM and obm.org, as well as between Linagora and linagora.com, and (iii) refrain 
 * from infringing Linagora intellectual property rights over its trademarks 
 * and commercial brands. Other Additional Terms apply, 
 * see <http://www.linagora.com/licenses/> for more details. 
 *
 * This program is distributed in the hope that it will be useful, 
 * but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY 
 * or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero General Public License 
 * for more details. 
 *
 * You should have received a copy of the GNU Affero General Public License 
 * and its applicable Additional Terms for OBM along with this program. If not, 
 * see <http://www.gnu.org/licenses/> for the GNU Affero General Public License version 3 
 * and <http://www.linagora.com/licenses/> for the Additional Terms applicable to 
 * OBM connectors. 
 * 
 * ***** END LICENSE BLOCK ***** */
package org.obm.push.backend;

import static org.fest.assertions.api.Assertions.assertThat;
import static org.obm.DateUtils.date;

import org.junit.Test;
import org.junit.runner.RunWith;
import org.obm.filter.SlowFilterRunner;
import org.obm.push.bean.SyncKey;
import org.obm.push.bean.change.item.ItemChange;
import org.obm.push.bean.change.item.ItemDeletion;

import com.google.common.collect.ImmutableList;

@RunWith(SlowFilterRunner.class)
public class DataDeltaTest {

	@Test
	public void testChangesWhenNull() {
		DataDelta dataDelta = DataDelta.builder()
				.changes(null)
				.syncKey(new SyncKey("123"))
				.syncDate(date("2012-01-01T10:22:33"))
				.build();
		
		assertThat(dataDelta.getChanges()).isNotNull().isEmpty();
	}

	@Test
	public void testChangesWhenEmpty() {
		DataDelta dataDelta = DataDelta.builder()
				.changes(ImmutableList.<ItemChange>of())
				.syncKey(new SyncKey("123"))
				.syncDate(date("2012-01-01T10:22:33"))
				.build();
		
		assertThat(dataDelta.getChanges()).isNotNull().isEmpty();
	}

	@Test
	public void testChangesWhenData() {
		DataDelta dataDelta = DataDelta.builder()
				.changes(ImmutableList.<ItemChange>of(new ItemChange("132")))
				.syncKey(new SyncKey("123"))
				.syncDate(date("2012-01-01T10:22:33"))
				.build();
		
		assertThat(dataDelta.getChanges()).containsOnly(new ItemChange("132"));
	}

	@Test
	public void testDeletionsWhenNull() {
		DataDelta dataDelta = DataDelta.builder()
				.deletions(null)
				.syncKey(new SyncKey("123"))
				.syncDate(date("2012-01-01T10:22:33"))
				.build();
		
		assertThat(dataDelta.getDeletions()).isNotNull().isEmpty();
	}

	@Test
	public void testDeletionsWhenEmpty() {
		DataDelta dataDelta = DataDelta.builder()
				.deletions(ImmutableList.<ItemDeletion>of())
				.syncKey(new SyncKey("123"))
				.syncDate(date("2012-01-01T10:22:33"))
				.build();
		
		assertThat(dataDelta.getDeletions()).isNotNull().isEmpty();
	}

	@Test
	public void testDeletionsWhenData() {
		DataDelta dataDelta = DataDelta.builder()
				.deletions(ImmutableList.<ItemDeletion>of(ItemDeletion.builder().serverId("132").build()))
				.syncKey(new SyncKey("123"))
				.syncDate(date("2012-01-01T10:22:33"))
				.build();
		
		assertThat(dataDelta.getDeletions()).containsOnly(ItemDeletion.builder().serverId("132").build());
	}
	
	@Test
	public void testMoreAvailableDefault() {
		DataDelta dataDelta = DataDelta.builder()
				.syncKey(new SyncKey("123"))
				.syncDate(date("2012-01-01T10:22:33"))
				.build();
		
		assertThat(dataDelta.hasMoreAvailable()).isFalse();
	}

	@Test
	public void testMoreAvailableFalse() {
		DataDelta dataDelta = DataDelta.builder()
				.syncKey(new SyncKey("123"))
				.syncDate(date("2012-01-01T10:22:33"))
				.moreAvailable(false)
				.build();
		
		assertThat(dataDelta.hasMoreAvailable()).isFalse();
	}
	
	@Test
	public void testMoreAvailableTrue() {
		DataDelta dataDelta = DataDelta.builder()
				.syncKey(new SyncKey("123"))
				.syncDate(date("2012-01-01T10:22:33"))
				.moreAvailable(true)
				.build();
		
		assertThat(dataDelta.hasMoreAvailable()).isTrue();
	}
}
