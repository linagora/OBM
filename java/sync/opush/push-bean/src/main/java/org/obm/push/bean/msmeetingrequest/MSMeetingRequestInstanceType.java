 ...
 */
public enum MSMeetingRequestInstanceType {
	
	SINGLE(0), 
	MASTER_RECURRING(1), 
	SINGLE_INSTANCE_RECURRING(2), 
	EXCEPTION_TO_RECURRING(3);

	private final int xmlValue;
	private static Map<Integer, MSMeetingRequestInstanceType> values;
	static {
		Builder<Integer, MSMeetingRequestInstanceType> builder = ImmutableMap.builder();
		for (MSMeetingRequestInstanceType type: values()) {
			builder.put(type.asXmlValue(), type);
		}
		values = builder.build();
	}
	
	private MSMeetingRequestInstanceType(int xmlValue) {
