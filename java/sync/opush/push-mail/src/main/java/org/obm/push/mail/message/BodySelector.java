...
public class BodySelector {
	...
	private Collection<MimePart> parts;
	private final List<String> mimeSubtypeInPriorityOrder;

	public BodySelector(MimePart root, List<String> mimeSubtypeInPriorityOrder) {
		this.mimeSubtypeInPriorityOrder = mimeSubtypeInPriorityOrder;
		parts = root.listLeaves(true, true);
	}

	public MimePart findBodyTextPart() {
		final MimePart part = findTextPart(parts, mimeSubtypeInPriorityOrder);
	...
	private MimePart lookForAlternativePart(final MimePart mimePart) {
		final MimePart mimePartParent = mimePart.getParent();
		if (mimePartParent != null && (mimePartParent.isMultipart() || mimePartParent.isNested()) && mimePartParent.getMultipartSubtype().equalsIgnoreCase("ALTERNATIVE")) {
			final Collection<MimePart> listParentParts = mimePartParent.listLeaves(true, true);
			for (String type: mimeSubtypeInPriorityOrder) {
				if (type.equalsIgnoreCase(mimePart.getSubtype())) {
					return mimePart;
				}
				final MimePart part = findTextPart(listParentParts, ImmutableList.of(type));
				if (part != null) {
					return part;
				}
			}
	...
	private MimePart findTextPart(Collection<MimePart> parts, List<String> mimeSubtypeInPriorityOrder) {
