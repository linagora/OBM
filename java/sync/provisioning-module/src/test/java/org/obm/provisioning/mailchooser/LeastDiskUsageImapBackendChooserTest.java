...
@GuiceModule(LeastDiskUsageImapBackendChooserTest.Env.class)
public class LeastDiskUsageImapBackendChooserTest implements H2TestClass {
	...
	@Inject
	private LeastDiskUsageImapBackendChooser chooser;
	@Inject
	...
	public void testChooseShouldSelectTheLeastLoadedImapBackend() {
		...
		ObmHost leastLoadedBackend = domain.getHostById(1);

		assertThat(chooser.choose(domain)).isEqualTo(leastLoadedBackend);
	}

	@Test
	public void testChooseShouldSelectTheLeastLoadedImapBackendEvenIfItHasNoMailboxes() throws SQLException {
		dbcp.getConnection().prepareStatement("DELETE FROM UserObm WHERE userobm_mail_server_id = 3").executeUpdate();
