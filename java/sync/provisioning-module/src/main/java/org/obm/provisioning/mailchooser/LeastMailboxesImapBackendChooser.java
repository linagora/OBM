...
@Singleton
public class LeastMailboxesImapBackendChooser implements ImapBackendChooser {
	...
	public ObmHost choose(ObmDomain domain) {
		try (Connection con = dbcp.getConnection();
			...
			int mailboxesCount = rs.getInt("count");
			...
			logger.info(String.format(
					"Host %s (%s) has %d mailboxe(s) and was chosen as the least loaded one.", host.getFqdn(), host.getIp(), mailboxesCount));
