...
public class GroupDaoJdbcImpl implements GroupDao {
	...
	public void addUser(ObmDomain domain, Group.Id groupId, ObmUser user) throws DaoException {
		try (Connection conn = obmHelper.getConnection();
				PreparedStatement ps = 
					conn.prepareStatement("INSERT INTO UserObmGroup (userobmgroup_group_id, userobmgroup_userobm_id) VALUES (?,?)")) {
	...
	private Set<Group.Id> getAllParentGroupIdsOfGroup(Connection con, Group.Id groupId, Set<Group.Id> alreadyAddedGroups) throws SQLException {
		...
		try (PreparedStatement ps = 
				con.prepareStatement("SELECT groupgroup_parent_id FROM GroupGroup WHERE groupgroup_child_id = ?")) {
	...
	private void updateGroupMappings(Connection con, Group.Id groupId) throws SQLException {
		...
		try (PreparedStatement ps = con.prepareStatement("DELETE FROM of_usergroup WHERE of_usergroup_group_id = ?")) {
			ps.setInt(1, groupId.getId());
			ps.executeUpdate();
		}
		if (!userIds.isEmpty()) {
			try (PreparedStatement ps = 
					con.prepareStatement("INSERT INTO of_usergroup (of_usergroup_group_id, of_usergroup_user_id) VALUES (?, ?)")) {
	...
	private Set<Integer> getAllUserIdsOfGroup(Connection con, Group.Id groupId) throws SQLException {
		...
		try (PreparedStatement ps = 
				con.prepareStatement("SELECT groupgroup_child_id FROM GroupGroup WHERE groupgroup_parent_id = ?")) {
