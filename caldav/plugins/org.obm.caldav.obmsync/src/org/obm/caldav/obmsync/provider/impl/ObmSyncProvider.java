...
public class ObmSyncProvider implements ICalendarProvider {
	...
	private ObmSyncProvider() {
		...
		String url = obmCalendarParameters.getPropertyValue("obmsync.server.url");
		this.cal = new CalendarLocator().locate(url);
	}

	public CalendarInfo getMyCalendar(AccessToken token, String userId) throws ServerFault,
			AuthFault {
		CalendarInfo[] listCalInfo = cal.listCalendars(token);
		for (CalendarInfo calInfo : listCalInfo) {
			if (calInfo.getMail().equals(token.getUser())) {
				return calInfo;
			}
		}
		return null;
	...
	public Event getEventFromExtId(AccessToken token, String userId, String uid) throws AuthFault,
			ServerFault {
		return cal.getEventFromExtId(token, getMyCalendar(token,userId).getUid(), uid);
	}

	public Event createEvent(AccessToken token, String userId, Event event) throws AuthFault,
			ServerFault {
	...
	@Override
	public List<Event> getListEventsFromIntervalDate(AccessToken token, String userId, Date start,
			Date end) throws AuthFault, ServerFault {
		return cal.getListEventsFromIntervalDate(token, getMyCalendar(token,userId).getUid(), start, end);
	}

	@Override
	public void updateParticipationState(AccessToken token, String userId, Event event, String participationState)
			throws AuthFault, ServerFault {
		for (Attendee att : event.getAttendees()) {
			if (getMyCalendar(token, userId).getMail().equals(att.getEmail())) {
				if (IEventService.PARTICIPATION_STATE_ACCEPTED.equals(participationState)) {
					att.setState(ParticipationState.ACCEPTED);
				} else if (IEventService.PARTICIPATION_STATE_DECLINED.equals(participationState)) {
					att.setState(ParticipationState.DECLINED);
				} else if (IEventService.PARTICIPATION_STATE_NEEDSACTION.equals(participationState)) {
					att.setState(ParticipationState.NEEDSACTION);
				}
			}
		}
		cal.modifyEvent(token, getMyCalendar(token,userId).getUid(), event, true);
	}

	@Override
	public String getParticipationState(AccessToken token, String userId, Event event) throws AuthFault, ServerFault {
		for(Attendee att : event.getAttendees()){
			if(getMyCalendar(token,userId).getMail().equals(att.getEmail())){
	...
	@Override
	public Set<CalendarInfo> getListCalendars(AccessToken token) throws ServerFault, AuthFault {

		Set<CalendarInfo> list = new HashSet<CalendarInfo>(	Arrays.asList(cal.listCalendars(token)));
		return list;
	}

	@Override
	public List<Event> getAllEvent(AccessToken token, String calendar) throws ServerFault, AuthFault {
		Date d = new Date();
		...
		return this.cal.getListEventsFromIntervalDate(token, calendar, start, end);
	}
}
