...
=item B<write_log( $text, $action, $level )>
...
=item level : niveau de debug associé au message. Si ce niveau est inférieur ou
égal au 'logLevel' indiqué dans le fichier de configuration, le message est
journalisé, sinon il est ignoré. Signification des différents niveaux :

=over 4

=item 0 : uniquement informations critiques (nom du script, erreurs provoquant l'arrêt, ...) ;

=item 1 : information sur les scripts en cours d'exécution ;

=item 2 : information avancées - niveau par défaut ;

=item 3 : niveau de debug faible ;

=item 4 : niveau de debug fort.
